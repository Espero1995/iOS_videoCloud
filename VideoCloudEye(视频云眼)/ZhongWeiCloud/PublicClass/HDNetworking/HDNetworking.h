...
typedef deviceCaptureReslult (^ _Nullable deviceCaptureBlock)(NSMutableDictionary * postDic, NSString * bIsEncryptStr,NSString * key);//设备抓图参数block
 ...
 *  @param failure    请求失败回调
 */
- (void)GET:(NSString *)URLString parameters:(NSDictionary *)parameters IsToken:(BOOL)isToken success:(Success)success failure:(Failure)failure;
 ...
 *  @param failure    请求失败回调
 */
- (void)POST:(NSString *)URLString parameters:(NSDictionary *)parameters IsToken:(BOOL)isToken success:(Success)success failure:(Failure)failure;
