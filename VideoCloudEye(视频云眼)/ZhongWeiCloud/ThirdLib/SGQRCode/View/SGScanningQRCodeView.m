...
/** 扫描内容的Y值 */
#define scanContent_Y self.frame.size.height * 0.2
/** 扫描内容的Y值 */
...
#pragma mark - - - 扫描外部View的创建
    ...
    CGFloat promptLabelY = scanContent_Y - 30;
    CGFloat promptLabelW = self.frame.size.width;
    CGFloat promptLabelH = 15;
    ...
    promptLabel.text = NSLocalizedString(@"请将设备二维码放入框内, 即可自动扫描", nil);
    [self addSubview:promptLabel];
    
    CGFloat light_buttonX = (iPhoneWidth - 36)/2;
    CGFloat light_buttonY = CGRectGetMaxY(scanContent_layer.frame) + 15;
    CGFloat light_buttonW = 36;
    CGFloat light_buttonH = 36;
    ...
    float qrCodeWayImgWidth = self.frame.size.width - 1.4*scanContent_X;
    float qrCodeWayImgHeight = qrCodeWayImgWidth/4;
    //扫描方式图片
    UIImageView *qrCodeWayImg = [[UIImageView alloc]init];
    qrCodeWayImg.image = [UIImage imageNamed:@"QrCodeWay"];
    qrCodeWayImg.frame = CGRectMake(0.7*scanContent_X, CGRectGetMaxY(self.light_button.frame) + 15, qrCodeWayImgWidth, qrCodeWayImgHeight);
    [self addSubview:qrCodeWayImg];
    
    
    
#pragma mark - - - 扫描边角imageView的创建
    // 左上侧的image
    UIImage *left_image = [UIImage imageNamed:@"ScanTopLeft"];//QRCodeTopLeft
    UIImageView *left_imageView = [[UIImageView alloc] init];
    CGFloat left_imageViewX = CGRectGetMinX(scanContent_layer.frame);//  CGRectGetMinX(scanContent_layer.frame)- left_image.size.width * 0.5 + margin
    CGFloat left_imageViewY = CGRectGetMinY(scanContent_layer.frame);//CGRectGetMinY(scanContent_layer.frame)- left_image.size.width * 0.5  + margin
    ...
    // 右上侧的image
    UIImage *right_image = [UIImage imageNamed:@"ScanTopRight"];//QRCodeTopRight
    UIImageView *right_imageView = [[UIImageView alloc] init];
    CGFloat right_imageViewX = CGRectGetMaxX(scanContent_layer.frame) -right_image.size.width;//CGRectGetMaxX(scanContent_layer.frame) - right_image.size.width * 0.5 - margin
    CGFloat right_imageViewY = left_imageView.frame.origin.y;
    CGFloat right_imageViewW = right_image.size.width;
    CGFloat right_imageViewH = right_image.size.height;
    ...
    // 左下侧的image
    UIImage *left_image_down = [UIImage imageNamed:@"ScanBottomLeft"];//QRCodebottomLeft
    UIImageView *left_imageView_down = [[UIImageView alloc] init];
    CGFloat left_imageView_downX = left_imageView.frame.origin.x;
    CGFloat left_imageView_downY = CGRectGetMaxY(scanContent_layer.frame)- left_image_down.size.height;//  CGRectGetMaxY(scanContent_layer.frame)- left_image_down.size.width * 0.5
    CGFloat left_imageView_downW = left_image_down.size.width;
    CGFloat left_imageView_downH = left_image_down.size.height;
    ...
    // 右下侧的image
    UIImage *right_image_down = [UIImage imageNamed:@"ScanBottomRight"];//QRCodebottomRight
    UIImageView *right_imageView_down = [[UIImageView alloc] init];
    CGFloat right_imageView_downX = right_imageView.frame.origin.x+1;
...
//手电筒
- (UIButton *)light_button
{
    if (!_light_button) {
        _light_button = [[UIButton alloc]init];
        
        [_light_button setBackgroundImage:[UIImage imageNamed:@"flashlightClose"] forState:UIControlStateNormal];
        [_light_button setBackgroundImage:[UIImage imageNamed:@"flashlightOpen"] forState:UIControlStateSelected];
        [_light_button addTarget:self action:@selector(light_buttonAction:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _light_button;
}
