...
/**
 定时器 用来自动播放图片
 */
static NSTimer * banner_timer;
@interface LocBannerView ()
<
    UIScrollViewDelegate
>
{
    //banNerView的宽和高 私有成员变量用下划线开头(书写习惯)
    CGFloat banner_width;
    CGFloat banner_height;
}

/**
 * @discussion 分页控件
 */
@property (nonatomic, strong) CustomPageControl *mainPage;

/**
 * @discussion ScrollView
 */
...
- (id)initWithFrame:(CGRect)frame andImageArray:(NSMutableArray *)addImageArray
{
    ...
    if (self && addImageArray.count > 2) {
        ...
        /**
         ...
         所以需要定义一个本类都能使用的成员变量
         */
        banner_width = frame.size.width;
        
        //或取bannerView 的高度
        banner_height = frame.size.height;
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
    ...
    if (banner_timer) {
        ...
        /**
         ...
         distantFuture(遥远的未来)
         */
        [banner_timer setFireDate:[NSDate distantFuture]];
...
#pragma mark - 滚动视图正在滚动 (拖拽过程中触发的方法)
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    self.mainPage.currentPage = scrollView.contentOffset.x / banner_width - 1;
}

#pragma mark - 滚动视图完成减速时调用 (就是手动拖拽完成后)
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (banner_timer) {
        ...
        /**
         ...
         延后2秒触发
         */
        [banner_timer setFireDate:[NSDate dateWithTimeIntervalSinceNow:3.0]];
        
        /**
         重新初始化定时器
         self.timer = [NSTimer scheduledTimerWithTimeInterval:3.0 target:self selector:@selector(timerFUNC:) userInfo:nil repeats:YES];
         */
    }
    ...
    //如果是数组内的最后一张图片 1
    if (currentPoint.x == (self.dataArray.count - 1) * banner_width) {
        
        //改变偏移量 显示数组内的第一张图片 1
        scrollView.contentOffset = CGPointMake(banner_width, 0);
    }
    ...
    if (currentPoint.x == 0) {
        ...
        //改变偏移量 显示数组内的 第二个图片6
        scrollView.contentOffset = CGPointMake((self.dataArray.count - 2) * banner_width, 0);
    }
    ...
    //改变分页控件上的页码
    self.mainPage.currentPage = newPoint.x / banner_width - 1;
}

#pragma mark - 初始化定时器
- (void)addTimer
{
    //初始化定时器 时间戳:3.0秒 目标:本类 方法选择器:timerFUNC 用户信息:nil 是否循环:yes
    banner_timer = [NSTimer scheduledTimerWithTimeInterval:3.0 target:self selector:@selector(timerFUNC:) userInfo:nil repeats:YES];
     ...
     */
    [[NSRunLoop currentRunLoop] addTimer:banner_timer forMode:NSRunLoopCommonModes];
    /**
...
- (void)timerFUNC:(NSTimer *)timer
{
    ...
    /**
     ...
     当前位置 + 一个屏幕宽度
     */
    CGFloat nextX = currentX + banner_width;
    
    /**
     判断滚动视图上将要显示的图片是最后一张时
     通过X值来判断 所以要 self.dataArray.count - 1
     */
    if (nextX == (self.dataArray.count - 1) * banner_width) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            //往第二张图片走
            self.mainScrollView.contentOffset = CGPointMake(self->banner_width, 0);
        }];
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            //改变对应的分页控件显示圆点
            self.mainPage.currentPage = self.mainScrollView.contentOffset.x / self->banner_width - 1;
        } completion:^(BOOL finished) {
...
#pragma mark - 加载分页控件
- (CustomPageControl *)mainPage
{
    if (!_mainPage) {
        
        //初始化分页控制器
        _mainPage = [[CustomPageControl alloc]initWithFrame:CGRectMake(50, 20, banner_width - 50 * 2, 20)];
        //分页控件上要显示的圆点数量
        ...
        //设置 滑动点(当前点) 的颜色
        _mainPage.currentPageIndicatorTintColor = MAIN_COLOR;
    }
...
- (UIScrollView *)mainScrollView
{
    if (!_mainScrollView) {
        ...
        //初始化滚动控件
        _mainScrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, banner_width, banner_height)];
         ...
         */
        _mainScrollView.contentSize = CGSizeMake(self.dataArray.count * banner_width, banner_height);
        ...
        //滚动视图的起始偏移量
        _mainScrollView.contentOffset = CGPointMake(banner_width, 0);
        ...
        for (int i = 0; i < self.dataArray.count; i ++) {
            ...
            //初始化图片视图
            UIImageView * imgV = [[UIImageView alloc]initWithFrame:CGRectMake(banner_width * i, 0, banner_width, banner_height)];
