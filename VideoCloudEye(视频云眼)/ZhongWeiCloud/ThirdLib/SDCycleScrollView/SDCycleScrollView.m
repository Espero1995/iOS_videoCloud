...
//    deleteBtn.frame = CGRectMake(self.bounds.size.width-18-8, 8, 18, 18);
//    [deleteBtn setBackgroundImage:[UIImage imageNamed:@""] forState:UIControlStateNormal];//close1//去掉轮播图关闭图片
...
- (void)deleteBtnClick
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(cycleScrollViewDeleteBanner)]) {
        [self.delegate cycleScrollViewDeleteBanner];
    }
}
