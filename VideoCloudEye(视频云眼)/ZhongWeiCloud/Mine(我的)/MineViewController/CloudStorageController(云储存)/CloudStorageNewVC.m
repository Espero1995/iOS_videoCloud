...
@implementation CloudStorageNewVC
//======================system========================
- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = NSLocalizedString(@"云存储", nil);
    self.view.backgroundColor = BG_COLOR;
    newTempArr = [NSMutableArray arrayWithCapacity:0];//初始化
    [self getCloudStorageList];
...
-(void)showHeadView{
    ...
    //标题
    [self.introduceView addSubview:self.headTitleLb];
    [self.headTitleLb mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    if (newTempArr.count == 0) {
        self.headTitleLb.text = NSLocalizedString(@"云存储", nil);
        //内容
        ...
        [contentLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(headImg.mas_right).offset(15);
            make.top.equalTo (self.headTitleLb.mas_bottom).offset(0);
            make.size.mas_equalTo(CGSizeMake(iPhoneWidth-90-30, 80));
        }];
    }else{
        self.headTitleLb.text  = NSLocalizedString(@"当前套餐", nil);
        MealRecordModel *model = newTempArr[0];
        ...
        [planInfoLb mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(headImg.mas_right).offset(15);
            make.top.equalTo (self.headTitleLb.mas_bottom).offset(5);
...
-(UILabel *)headTitleLb{
    if (!_headTitleLb) {
        ...
        _headTitleLb.font = [UIFont systemFontOfSize:17.0f];
    }
    return _headTitleLb;
}
