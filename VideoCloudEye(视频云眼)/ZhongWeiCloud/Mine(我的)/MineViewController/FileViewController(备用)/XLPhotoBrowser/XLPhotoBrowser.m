...
@property (nonatomic, strong) NSArray *imagesDetails;
@end
...
- (UIWindow *)photoBrowserWindow
{
    if (!_photoBrowserWindow) {
        ...
        _photoBrowserWindow.windowLevel = 2000;//MAXFLOAT
...
- (void)setupToolBar {
    ...
    [backButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.indexLabel.mas_top).offset(0);
        make.bottom.equalTo(self.indexLabel.mas_bottom).offset(0);
        make.left.equalTo(self.mas_left).offset(10);
    }];
    
    _toolBar = [[UIView alloc] initWithFrame:CGRectMake(0, self.frame.size.height-100, self.frame.size.width, 100)];
    [_toolBar setBackgroundColor:[[UIColor blackColor]colorWithAlphaComponent:0.5]];
    ...
    self.detailsLabel.font = FONT(13);
    [self.toolBar addSubview:self.detailsLabel];
    [self.detailsLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.toolBar.mas_top).offset(1);
        make.left.equalTo(self.toolBar.mas_left).offset(5);
        make.right.equalTo(self.toolBar.mas_right).offset(-5);
...
//    [deleteBtn addTarget:self action:@selector(createDeleteAlart) forControlEvents:UIControlEventTouchUpInside];
//    [_toolBar addSubview:deleteBtn];
    //分享按钮
    UIButton *shareBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    shareBtn.frame = CGRectMake(_toolBar.frame.size.width-55, _toolBar.frame.size.height - 35, 50, 28);
    ...
    saveBtn.frame = CGRectMake(_toolBar.frame.size.width-100, _toolBar.frame.size.height - 35, 50, 28);
...
- (void)createDeleteAlart{
    ...
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"删除提醒"message:@"确定删除该图片？删除后无法恢复！"preferredStyle:UIAlertControllerStyleAlert];
    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
        ...
        [defaults setInteger:self.currentImageIndex forKey:@"deleteImageIndex"];
...
//    UIImage * image = zoomingScrollView.currentImage;
...
- (void)setup{
    ...
    _backButton.frame = CGRectMake(0, iPhoneHeight-130,iPhoneWidth,iPhoneHeight-130);
    _backButton.alpha = 0;
    ...
    CGFloat spacing = 0;
    if (iPhone_5_Series) {
        width = (self.bounds.size.width-140)/3;
        spacing = 35;
    }else{
        width = (self.bounds.size.width-200)/3;
        spacing = 50;
    }

    UIView * whiteView = [[UIView alloc]initWithFrame:CGRectMake(0, self.bounds.size.height - 130, self.bounds.size.width, 130)];
    whiteView.backgroundColor = [UIColor whiteColor];
    [self.backView addSubview:whiteView];
    
    UIView * lineView = [[UIView alloc]initWithFrame:CGRectMake(0, 25+width, iPhoneWidth, 1)];
    lineView.backgroundColor = [UIColor grayColor];
    [whiteView addSubview:lineView];
    UIButton * cancleBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    cancleBtn.frame = CGRectMake(0, width+7, self.bounds.size.width, 140-width);
    ...
    for (int i = 0; i < shareIconArray.count; i ++) {
        ...
        itemView.frame = CGRectMake(spacing*(i+1)+i*width,10,width,width+10);
        ...
        title.frame = CGRectMake(5, width-20, width-10,20);
...
- (void)hide{
    ...
    [UIView animateWithDuration:0.4 animations:^{
        self.backView.frame = CGRectMake(0,iPhoneHeight,iPhoneWidth,200);
...
- (void)share:(id)sender{
    ...
    if (button.tag==0) {
        NSData *dataimg =UIImageJPEGRepresentation(image,0.8);
        if (![TencentOAuth iphoneQQInstalled]) {
            NSLog(@"请移步 Appstore 去下载腾讯 QQ 客户端");
            [XHToast showCenterWithText:@"你还没有安装QQ！"];
        }else{
        
            //QQ 分享图片不超过 1M ，没有压缩的必要
//            NSData *data = UIImagePNGRepresentation(image);
            QQApiImageObject *imgObj = [QQApiImageObject objectWithData:dataimg previewImageData:dataimg  title:@"视频云眼" description:@"截图"];
            
            SendMessageToQQReq *req = [SendMessageToQQReq reqWithContent:imgObj];
            NSLog(@"req::%@",req.message);
            ...
            if (code == 0) {
                ...
                [XHToast showCenterWithText:@"分享失败"];
            }
        }
...
//        NSLog(@"分享QQ");
        ...
        if ([WXApi isWXAppInstalled] && [WXApi isWXAppSupportApi]) {
            ...
            [XHToast showCenterWithText:@"你还没有安装微信！"];
        }
...
//            [self updatePageControlIndex];
        ...
        NSLog(@"%ld",(long)self.currentImageIndex);
...
- (void)updateIndexContent
{
    ...
    self.detailsLabel.text = self.imagesDetails[self.currentImageIndex];
    ...
    [attributedString addAttribute:NSParagraphStyleAttributeName value:paragraphStyle range:NSMakeRange(0, [self.detailsLabel.text length])];
    [self.detailsLabel setAttributedText:attributedString];
    self.detailsLabel.lineBreakMode = NSLineBreakByCharWrapping;
}
 ...
 */
+ (instancetype)showPhotoBrowserWithImages:(NSArray *)images andImagesName:(NSArray *)imagesName currentImageIndex:(NSInteger)currentImageIndex
{
//    NSLog(@"andImagesName:%@",imagesName);
    ...
    browser.imagesDetails = imagesName;
    [browser show];
    return browser;
}
