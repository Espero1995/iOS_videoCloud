...
/**
 * description: 这里有两个关于录像回放的接口 (1).v1/media/record/playback(录像回放) (2).v1/media/record/playback_ctrl(录像回放控制)
 * 注意(1)接口会传个时间范围的，而这个时间范围从此接口获取(v1/media/record/list)取其第一段的startTime即可(若取0点可能会不适配有些设备，会黑屏,且播放失败)，而endTime取当天的23:59:59即可。
 * 而(2)接口则会传一个时间戳来控制当前录像的播放进度啥的，但是前提就是【该时间戳必须在(1)接口的时间范围之内，否则出错】
 */
...
//========View========
#import "SZCalendarPicker.h"//时间选择器
#import "CenterTimeView.h"
#import "LoadingHubView.h"//动画加载View
//警告框
...
@implementation ChannelPlayBackVC
{
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    dateTipLb1.text = NSLocalizedString(@"通道消息", nil);
    ...
    if (_arrayM.count != 0) {
        ...
        if (![NSString isNull:model.alarmPic]) {
            ...
            [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse * _Nullable response, NSData * _Nullable data, NSError * _Nullable connectionError) {
                ...
                self.key = self.channelModel.dev_p_code;
                self.bIsEncrypt = self.channelModel.enableSecLib;
                ...
                if (len %16 == 0 && [((NSHTTPURLResponse *)response) statusCode] == 200 && self.key.length > 0 && self.bIsEncrypt) {
                    ...
                    if (decrptImageSucceed == 1) {
                        ...
                        if (image) {
                            ...
                            [self defalutImage:cell ImageUrl:imaUrl];
                        }
                    }else{
                        [self defalutImage:cell ImageUrl:imaUrl];
                    }
...
- (void)defalutImage:(AlarmNewCell *)cell ImageUrl:(NSURL *)imaUrl
{
...
//    [_arrayM removeAllObjects];
    ...
    NSLog(@"self.channelModel:%@",self.channelModel.deviceId);
    [postDic setObject:self.channelModel.deviceId forKey:@"dev_id"];
...
- (void)getRecordVideo:(NSString *)timeStr//获取的是日期
{
    ...
    if (self.bIsAP) {
        ...
        if (self.isDeviceVideo == NO) {
            [self.videoManage searchChannelPlayBackVideoTimeListRecVideoType:JWRecVideoTypeStatusCenter beginTime:date completionBlock:^(NSArray *recVideoTimeArr, JWErrorCode errorCode) {
            ...
            [self.videoManage searchChannelPlayBackVideoTimeListRecVideoType:JWRecVideoTypeStatusLeading beginTime:date completionBlock:^(NSArray *recVideoTimeArr, JWErrorCode errorCode) {
                if (errorCode == JW_SUCCESS || errorCode == JW_SUCCESS_SEARCH_VIDEO_LIST) {
                    //回放时间数组
                    if (recVideoTimeArr.count>0) {
                        for (int i = 0; i < recVideoTimeArr.count; i++) {
...
- (void)autoSearchAndPlayVideoIsDeviceVideo:(BOOL)isDeviceVideo TimeStr:(NSString *)timeStr
{
    ...
    [self.videoManage searchChannelPlayBackVideoTimeListRecVideoType:isDeviceVideo ? JWRecVideoTypeStatusLeading :JWRecVideoTypeStatusCenter beginTime:date completionBlock:^(NSArray *recVideoTimeArr, JWErrorCode errorCode) {
...
//                                NSLog(@"开始时间戳：%d    \n结束时间戳：%d\n时间差：%d\n起始位置：%f\n结束位置：%f",model.startTimestamp,model.endTimestamp,model.timeInterval,model.startLoc,model.endLoc);
                            ...
                            NoTimeShaftRulerModel *model = self.rulerModelArr[0];
//                            NSLog(@"时间管理大师二:%d",model.startTimestamp);
                            self.noTimeShaftRuler.timeLb.text = [unitl timeStampSwitchTime:model.startTimestamp andFormatter:@"HH:mm:ss" IsTenBit:YES];
...
- (IBAction)btnRatioClick:(id)sender
{
    ...
    __block ChannelPlayBackVC *blockSelf = self;
...
- (void)createAlertController
{
    ...
    if(self.isFull){
        ...
        customV.bounds = CGRectMake(0, 0, iPhoneHeight, iPhoneWidth);
...
- (void)postChangePlaySpeed:(float)SpeedValue
{
    ...
    [postDic setObject:self.channelModel.chanCode forKey:@"chan_code"];
...
//            NSString *beginTimeStr =[NSString stringWithFormat:@"%@ %@",self.timeStr,self.timeMinStr];
           // NSDate *date = [formatter dateFromString:beginTimeStr];
            ...
            NSDate *startDate;
            //如果存在定位时间，就采用定位时间，否则就按照获取到的list的第一个startTime
            if(self.timeMinStr){
                NSString *tempTimeStr = [NSString stringWithFormat:@"%@ %@",self.timeStr,self.timeMinStr];
                NSInteger tempTimeInteger = [unitl timeSwitchTimeStamp:tempTimeStr andFormatter:@"YYYY-MM-dd HH:mm:ss"];
                startDate = [NSDate dateWithTimeIntervalSince1970:tempTimeInteger];
                
            }else {
                if (isDefaultMode) {
                    //去拿list接口数组的第一段开始时间
                    if (self.rulerModelArr && self.rulerModelArr.count > 0) {
                        NoTimeShaftRulerModel *model = self.rulerModelArr[0];
                        startDate = [NSDate dateWithTimeIntervalSince1970:model.startTimestamp];
                    }
                    
                }else {
                    if (self.videoTimeModelArr && self.videoTimeModelArr.count > 0) {
                        ZCVideoTimeModel *model = self.videoTimeModelArr[0];
                        NSInteger offTimeStamp = [[NSString stringWithFormat:@"%f",model.benginTime] integerValue];
                        NSString *tempTimeStr = [NSString stringWithFormat:@"%@ 00:00:00",self.timeStr];
                        NSInteger nowDayTimeStampInt = [unitl timeSwitchTimeStamp:tempTimeStr andFormatter:@"YYYY-MM-dd HH:mm:ss"];
                        NSInteger beginTimeStamp = nowDayTimeStampInt + offTimeStamp;
                        startDate = [NSDate dateWithTimeIntervalSince1970:beginTimeStamp];
                    }
                }
            }
            
            NSLog(@"时间管理大师之【调试】滑动时间轴，得到播放的【开始】时间是：%@===【结束】时间:%@",startDate,endDate);

//            NSLog(@"时间管理大师之-----------多人运动数组:%@",self.rulerModelArr);
            ...
            [self.videoManage JWStreamChannelPlayerManageWithVideoType:videoType startTime:startDate endTime:endDate BIsEncrypt:self.channelModel.enableSecLib Key:self.channelModel.dev_p_code completionBlock:^(JWErrorCode errorCode) {
                if (errorCode == JW_SUCCESS) {
                    //注意：这里不是逻辑错误。
                    ...
                    NSLog(@"LB:%@=====%@",self.centerTimeView.btn_time.titleLabel.text,tempTimeStr);
                    NSLog(@"【调试】滑动时间轴，得到播放的【开始】:%@===%@",beginTimeStr,self.videoManager.delegate);
                    self.btn_start.selected = YES;
                    self.btn_hd.selected = NO;
                    NSLog(@"回放成功了");
                }if (errorCode == JW_FAILD) {
...
- (void)handleSwipeFrom:(UISwipeGestureRecognizer *)recognizer
{
    if (recognizer.enabled) {
        ...
        //TODO 这个是靠通知去更新最新时间的，但是现在的版本不支持返回最新时间 这个后面改下即可
...
-(void)shupinAction
{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        [self.VideoViewBank mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.topMargin.mas_equalTo(self.view.mas_topMargin);
            make.left.right.mas_equalTo(self.view);
...
{
    if ([[unitl currentTopViewController]isKindOfClass:[ChannelPlayBackVC class]]) {
       [self getRecordVideo:self.timeStr];
    }else{
