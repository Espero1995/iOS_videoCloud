...
@interface SharePermissionDetailVC ()
<
    UITableViewDelegate,
...
@implementation SharePermissionDetailVC
...
//    if (self.PermissionDetailDic) {
//
//        if (self.featureModel.isTalk == 1) {//判断设备是否支持对讲
//            if ([[self.PermissionDetailDic objectForKey:@"talk"] intValue] == 1) {
//                isTalk = FeatureOpen;
//            }else{
//                isTalk = FeatureClose;
//            }
...
//
//        //录像回放
//        if ([[self.PermissionDetailDic objectForKey:@"hp"] intValue] == 1) {
...
//            if ([[self.PermissionDetailDic objectForKey:@"ptz"] intValue] == 1) {
...
//
//        //告警消息
//        if ([[self.PermissionDetailDic objectForKey:@"alarm"] intValue] == 1) {
//            isAlarm = FeatureOpen;
//        }else{
...
- (void)completeClick
{
    ...
    //设置完之后model要修改
    self.shareModel.rtv = @"1";
    self.shareModel.volice = @"1";
    self.shareModel.hp = [NSString stringWithFormat:@"%@",isPlayback];
    self.shareModel.alarm = [NSString stringWithFormat:@"%@",isAlarm];
    self.shareModel.talk = [NSString stringWithFormat:@"%@",isTalk];
    self.shareModel.ptz = [NSString stringWithFormat:@"%@",isCloudDeck];
    ...
    if (self.shareModel.timeLimit == 0) {
        //时限
...
{
//    self.shareModel = [ShareFeatureModel mj_objectWithKeyValues:self.dev_mList.ext_info[@"shareFeature"]];
    self.cmsModel = self.dev_mList.ext_info.feature_cms;
//    [self setDeviceFeature:self.dev_mList];
    //云台
    if (self.featureModel.isCloudDeck == 1) {//支持云台
        if ([self.shareModel.ptz intValue] == 1) {//开启
            isCloudDeck = FeatureOpen;
        }else{//关闭
            isCloudDeck = FeatureClose;
        }
    ...
    if (self.featureModel.isTalk == 1) {//支持对讲
        if ([self.shareModel.talk intValue] == 1) {//开启对讲
            isTalk = FeatureOpen;
        }else{//关闭
            isTalk = FeatureClose;
        }
    ...
    //历史回放（均支持）
    if ([self.shareModel.hp intValue] == 1) {//开启
        isPlayback = FeatureOpen;
    }else{//关闭
        isPlayback = FeatureClose;
    }
    ...
    //报警推送（均支持）
    if ([self.shareModel.alarm intValue] == 1) {//开启
        isAlarm = FeatureOpen;
    }else{
        isAlarm = FeatureClose;
    }
