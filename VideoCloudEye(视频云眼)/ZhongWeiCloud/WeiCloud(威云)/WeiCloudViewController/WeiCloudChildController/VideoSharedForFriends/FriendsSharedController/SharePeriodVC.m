...
//    [XHToast showCenterWithText:tempStr];
    ...
    if ([self getFrontofString:self.startTime] == [self getFrontofString:self.endTime]) {
        if ([self getBackofString:self.startTime] > [self getBackofString:self.endTime]) {
            [XHToast showCenterWithText:NSLocalizedString(@"起始时间不能大于等于结束时间", nil)];
            return;
        }
        if ([self getBackofString:self.startTime] == [self getBackofString:self.endTime]) {
    ...
    if (self.modifyPermissionDic) {
        ...
        [postDic setObject:[self.modifyPermissionDic objectForKey:@"timeLimit"] forKey:@"timeLimit"];//时限
    }else{
        ...
        if (ret == 0) {
            ...
            for (int i = 0; i < devListArr.count; i++) {
                if ([((dev_list *)devListArr[i]).ID isEqualToString:self.dev_mList.ID]) {
                    ...
                    appointDevModel.ext_info.shareFeature.startTime = postDic[@"startTime"];
                    appointDevModel.ext_info.shareFeature.endTime = postDic[@"endTime"];
                }
            }
